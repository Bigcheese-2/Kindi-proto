...
export class ExportService {
   ...
   * @param data Data to export
   * @param options Export options with annotation flag
   */
  async exportWithAnnotations(
    data: any,
    options: ExportOptions & { includeAnnotations?: boolean }
  ): Promise<Blob | string> {
    // If annotations should be included, we would integrate with the annotation service here
    // For now, we'll just pass through to the regular export
  ...
  private async exportGraphAsImage(
    ...
    const canvas = await html2canvas(graphRef.current as HTMLElement);
  ...
  private exportGraphAsSVG(
    ...
    // Find SVG element in the graph component
  ...
  private async exportMapAsImage(
    ...
    const canvas = await html2canvas(mapRef.current as HTMLElement);
    return new Promise<Blob>((resolve) => {
      canvas.toBlob(
        (blob) => resolve(blob as Blob),
        `image/${options.format}`,
...
// Utility function to save exported content
export const saveExportedContent = (
  content: Blob | string,
  filename: string
): void => {
  if (typeof content === 'string') {
    // For string content (SVG, CSV, JSON)
